...
//      log.info("checkIn( %d ) pos %d end %d bufferlegth %d", size, bufferpos, end, buffer.length);
      if (size > end - bufferpos) {
         if (size > buffer.length) {
...
//            log.info("checkin exception pos %d end %d", bufferpos, end);
   ...
   public int shift() {
      if (bufferpos == 0) {
         if (end > 0) {
            log.info("shift about to fail pos %d offset %d ceiling %d end %d size %d",
                    bufferpos, offset, ceiling, end, this.buffer.length);
   ...
   /**
    * Fill buffer while reading string. The current position till possible match
    * is stored in the current pos read.
    *
    * @param p
    * @return
    */
