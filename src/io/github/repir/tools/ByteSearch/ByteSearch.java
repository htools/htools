...
/**
 * Factory for fast search of a byte array needle in a byte array. The input pattern
 * is considered to be a ByteRegex expression. Depending on the pattern, a {@link ByteSearchSingle}
 * is used for single characters (e.g. ">", "a"), a {@link ByteSearchSingleClass} is used for single
 * characters that have several possible matches (e.g. "\\s", "[a-z]"), a {@link ByteSearchString}
 * is used when the pattern contains multiple characters but no decision (e.g. "<link"), and
 * a {@link ByteRegex} is used for more complex needles (e.g. "\\s*\\w").
 *
