...
public class Tabular {
   ...
   public Tabular( String columnids ) {
     this( columnids, new TableDefault()); 
   }
   
   public Tabular(String columnids, TableTemplate template) {
      this.template = template;
      ArrayList<Pos> findAll = sep.findAll(columnids);
      Pos last = null;
      for (Pos p : findAll) {
   ...
   public void addColumn(String s, boolean left, boolean right) {
      s += "::::";
      String p[] = s.split(":", -1);
      Column c = new Column(p[0], p[1], p[2], left, right);
      columns.add(c);
      if (p[3].length() > 0) {
         Class clazz = ClassTools.toClass(p[3], ColumnFormatter.class.getPackage().getName());
         Constructor cons = ClassTools.getAssignableConstructor(clazz, ColumnFormatter.class);
         ColumnFormatter cf = (ColumnFormatter) ClassTools.construct(cons);
         c.setFormatter(cf);
      }
   }
      ...
      for (Row row : rows) {
         ...
         for (int i = 0; i < columns.size(); i++) {
            if (row.cells[i] != null) {
               ...
               if (c.colspan > 1) {
                  Column last = columns.get(i + c.colspan - 1);
                  String columndef = (i == 0 && column.leftbar ? "|" : "") + getAlign(c.align()) + (last.rightbar ? "|" : "");
                  r.append(PrintTools.sprintf("\\multicolumn{%d}{%s}{%s}", c.colspan, columndef, c.value));
                  i += c.colspan - 1;
               } else {
                  r.append(c.value);
               }
      ...
      if (hastop) {
         ...
         for (int i = 0; i < columns.size(); i++) {
            ...
            if (c.top != null) {
               ...
               sb.append(PrintTools.sprintf("\\multicolumn{%d}{%s}{%s}", end - i, columndef, c.top));
               if ((end - i) > 1) {
                  i = end - 1;
               }
               if (i + 1 < columns.size()) {
   ...
   private String getColumnHeaders() {
      ...
      for (int i = 0; i < columns.size(); i++) {
         ...
         if (c.header != null) {
            ...
            String columndef = (i == 0 && c.leftbar ? "|" : "") + "c" + (columns.get(end - 1).rightbar ? "|" : "");
            sb.append(PrintTools.sprintf("\\multicolumn{%d}{%s}{%s}", end - i, columndef, c.header));
            if ((end - i) > 1) {
               i = end - 1;
            }
            if (i + 1 < columns.size()) {
   ...
   public class Column {
      ...
      ColumnFormatter formatter = template.columDefault();

      public Column(String id, String header, String top, boolean leftbar, boolean rightbar) {
         this.id = id;
