...
public abstract class StructuredStream {
   ...
   private void setStructurePos(Field e) {
      ...
      if (nextField == null) {
         log.fatal("Trying to move to an unknown label %s in %s", e.label[0], this.toString());
      }
   }

   private void setStructurePos(String e) {
      for (nextField = start; nextField != null && !e.equals(nextField.label[0]); nextField = nextField.next);
      if (nextField == null) {
   ...
   protected Field find(FieldType type, String label) {
      Field e = start;
      for (; e != null && (e.type != type || !e.label[0].equals(label)); e = e.next);
      return e;
   }
   ...
   protected String getCurrentLabel() {
      if (nextField != null) {
         return nextField.label[0] + " " + nextField.type.toString();
      }
      return "END";
   }
   ...
   protected void checkRead(Field e) {
      ...
      if (nextField != e) {
         log.fatal("Invalid Structure trying to read/write data labelled %s in %s", e.label[0], this.toString());
      }
...
//   }
    ...
    * Field can be added to the structure. Values can be written/read using the
   ...
   protected abstract class Field {
       ...
       * <p/>
       * @throws EOFException when EOF or ceiling is encountered
       */
      public abstract void readNoReturn() throws EOFException;
       ...
       * @throws EOFException when EOF or ceiling is encountered,
       */
      public abstract void skip() throws EOFException;
   ...
   protected class StartField extends Field {
      ...
      protected StartField() {
         ...
         this.label = new String[]{"StartField"};
         last = this;
         last.next = this;
      }

      public void readNoReturn() throws EOFException {
         throw new EOFException("StartField should never be read");
      }
