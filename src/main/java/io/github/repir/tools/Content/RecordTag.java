 ...
 * buffer. When reading tuples, the end is reached on EOF, which is also
 * triggered if a preset ceiling is encountered.
 * <p/>
 ...
 * {@link StructuredDataStream.Field}. Override {@link #initStructure()} to add
 * the fields to the TupleData structure. The order in which the fields are
 * created define the fixed order in which these fields are written and read in
 * the data stream. For some implementations like {@link RecordTag} fields can
 * be optional, and so can be omitted, but other implementations such as
 * {@link RecordCSV} require all fields to be written/read because no unique
 * field identifiers are used.
 * <p/>
 * <
 * p/>
 * @author jeroen
 */
public abstract class RecordTag extends StructuredTagStream2 {

   public Log log = new Log(RecordTag.class);
   Datafile datafile;
   int tupleswritten;
    ...
    * @param datafile
    */
   public RecordTag(Datafile datafile) {
      super();
    ...
    * of tuples written.
    */
   public RecordTag(BytesOut bytes) {
      super(new BufferReaderWriter(bytes));
   }
    ...
    * @param tuple
    */
   public void hookRecordWritten(RecordTag tuple) {
   }
