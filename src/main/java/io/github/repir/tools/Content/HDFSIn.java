...
public class HDFSIn implements DataIn {
   ...
   public HDFSIn(FileSystem fs, Path path) {
      try {
         ...
         //log.info("open %s bufersize %d", path.getName(), buffersize);
   ...
   public static long getLength(FileSystem fs, Path path) {
      try {
         return fs.getFileStatus(path).getLen();
      } catch (IOException ex) {
         log.fatalexception(ex, "getLength( %s, %s)", fs, path);
      }
      return 0;
   }
   ...
   public void fillBuffer(BufferReaderWriter buffer) throws EOFException {
      ...
      //log.info("fillBuffer() %s bufferoffset %d bufferpos %d bufferend %d ceiling %d buffersize %d maxread %d read %d",
   ...
   public void setBufferSize(int buffersize) {
      buffer.offset += buffer.resize(buffersize);
   }
