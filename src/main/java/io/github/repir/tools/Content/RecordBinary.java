...
/**
 * The RecordBinary class provides a toolkit to store data in a datafile that
 ...
 * <p/>
 * To use: create a new class that extends RecordBinary. For the fields, create
 ...
 * add the fields to the RecordBinary structure, in that order.
 * <p/>
 * @author jeroen
 */
public abstract class RecordBinary extends StructuredDataStream {

   public Log log = new Log(RecordBinary.class);
    ...
    * @param datafile
    */
   public RecordBinary(Datafile datafile) {
      super();
    ...
    */
   public RecordBinary(byte[] bytes) {
    ...
    * of tuples written.
    */
   public RecordBinary(BytesOut bytes) {
      super(new BufferReaderWriter(bytes));
   }
   ...
   /**
    * @return {@link Datafile} that the RecordBinary class read/writes to.
    */
    ...
    * know the exact offset at which a RecordBinary starts.
    * <p/>
    * @param offset
    */
   ...
   public Datafile getSegment(int partition) {
      return datafile.getSubFile(io.github.repir.tools.Lib.PrintTools.sprintf(".%04d", partition));
   }
   ...
   public long[] mergeSegments() {
      ...
      long offsets[] = dir.mergeFiles(new Datafile(datafile.getFileSystem(), datafile.getFullPath()), datafile.getFilename());
      return offsets;
   }
   ...
   /**
    * clones the RecordBinary structure from another RecordBinary instance
    * <p/>
    * @param source tuple whose structure is being cloned
    */
   public void copyStructure(RecordBinary source) {
   ...
   /**
    * clones this RecordBinary object, by simply instantiating a new
    * RecordBinary object using the same Datafile
    * <p/>
    * @return a clone of this RecordBinary object
    */
   @Override
   public RecordBinary clone() {
      RecordBinary tuple = null;
      try {
         Constructor<? extends RecordBinary> declaredConstructor = this.getClass().getDeclaredConstructor(Datafile.class);
    ...
    * means a whole row was read/written. In RecordBinary the structure
    ...
    * for sorted implementations of RecordBinary, allowing them to cache & sort
   ...
   /**
    * open a RecordBinary in writing mode.
    */
   ...
   /**
    * open a RecordBinary in reading mode
    */
   ...
   /**
    * closes the RecordBinary stream.
    */
   ...
   /**
    * @param loadfactor determines the capacity needed for a table that contains n tuples
    * to avoid too many collisions in hash tables.
    */
