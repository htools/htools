 ...
 * <p/>
 * A descriptive fatal exception is thrown if the arguments do not match the
...
public class ArgsParser {
   ...
   static final ByteRegex combi = new ByteRegex(optional, repeat, term);
   final String argumentstring;
   final HashMap<String, String> parsedargs = new HashMap<String, String>();
   ...
   public ArgsParser(String args[], String message) {
      ...
      ArrayList<ByteRegex.Pos> findAll = combi.findAll(bmessage, 0, bmessage.length);
      int argspos = 0;
      for (int posi = 0; posi < findAll.size(); posi++) {
         Pos p = findAll.get(posi);
         switch (p.pattern) {
            case 2:
   ...
   public String[] getRepeatedGroup() {
      if (repeatedgroup == null)
         log.fatal("argument list does not contain a repeated group: %s", argumentstring);
