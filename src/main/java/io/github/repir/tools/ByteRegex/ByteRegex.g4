...
startRule
   : (QUES)? 
     (QUOTES { byteregex.quotesafe = true;
             } 
      | CASE { byteregex.casesensitive = true; } 
      )?
     (frag { byteregex.root = $frag.fragout.start;
...
bracket returns [ Node node ]
   ...
   | ( BLOCKOPEN
     character { $node = new Node( TYPE.CHAR, byteregex.casesensitive );
                 ($node).next = new Node[1];
                 if ($character.node.type == TYPE.START) {
                    positive=false;
                 } else {
                    lastcharacter = $character.node.allowedChar();
                    $node.setAllowed( lastcharacter );
                 }
               }
     ( ( MINUS character { 
       ...
       | character { 
         ...
         lastcharacter = $character.node.allowedChar();
         $node.combineAllowed( $character.node.allowed );
      } )* BLOCKCLOSE )
;

character returns [ Node node ]
   @init { $node = new Node( TYPE.CHAR, byteregex.casesensitive );
           ($node).next = new Node[1];
         }
