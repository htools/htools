...
/**
 * Modified version of HBase FileOutputFormat2 using the instructions at
 * http://tech.adroll.com/blog/data/2014/07/15/multi-table-bulk-import.html to
 * support multi-table bulk loading. This class supports faster large volume 
 * bulk loads into HBase tables. Row keys should be prefixed with the table
 * name to sort out different tables. In the job, setup the configuration by
 * using configureIncrementalLoad() with the tables that will be written to. In
 * the Mapper, use the static makeKey() methods to create a valid row key.
 *
 * @author Jeroen
 */
public class BulkOutputFormat extends FileOutputFormat<ImmutableBytesWritable, Cell> {

    public static Log log = new Log(BulkOutputFormat.class);

    static org.apache.commons.logging.Log LOG = LogFactory.getLog(HFileOutputFormat2.class);
...
// -- This section contains our utility functions to map <table>:<row> to/from bytes --
    /**
     * Parses an ImmutableBytesWritable into a String array of two elements. The
     * first element is the table and the second is the row.
     *
    ...
    protected static String[] getTableAndRow(ImmutableBytesWritable bytes) {
        ...
        if (index == -1) {
            log.fatal("Output key has an invalid format %s", keyWithPrefix);
    ...
    protected static String getTable(ImmutableBytesWritable bytes) {
        String[] tableAndRow = getTableAndRow(bytes);
        return tableAndRow[0];
    }

    protected static String getRow(ImmutableBytesWritable bytes) {
        String[] tableAndRow = getTableAndRow(bytes);
        return tableAndRow[1];
    }

    /**
     * Concatenates the input table and row together, using a colon as the
     * separator
     *
     * @param table
     * @param row
    ...
    public static byte[] makeKey(String table, byte[] row) {
        return makeKey(Bytes.toBytes(table), row);
    }

    public static byte[] makeKey(byte[] table, byte[] row) {
        return ByteTools.concatenate(table, separator, row);
    }
    ...
    static <V extends Cell> RecordWriter<ImmutableBytesWritable, V>
        ...
        return new RecordWriter<ImmutableBytesWritable, V>() {
            ...
            public void write(ImmutableBytesWritable row, V cell)
                ...
                String tableName = tableAndKey[0].replaceAll(":", "_");
     ...
     */
    private static List<ImmutableBytesWritable> getRegionStartKeys(HTable... tables)
            throws IOException {
        ArrayList<ImmutableBytesWritable> ret
                = new ArrayList<ImmutableBytesWritable>();
        for (HTable table : tables) {
            byte[][] byteKeys = table.getStartKeys();
            for (byte[] byteKey : byteKeys) {
                byte[] reducerKey = makeKey(table.getTableName(), byteKey);
                ret.add(new ImmutableBytesWritable(reducerKey));
            }
        }
        return ret;
    }
     ...
     */
    public static void configureIncrementalLoad(Job job, HTable... tables)
            throws IOException {
        ...
        for (ImmutableBytesWritable key : allStartKeys) {
            log.info("key %s", Bytes.toString(key.get()));
        }
        /**
         * END ADROLL CODE
         */
